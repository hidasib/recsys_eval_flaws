...
def preprocess_data(path:str, review_tsv_file:str):
    if not os.path.exists(review_tsv_file):
        raw_file_name = "steam_reviews.json"
        ...
        print("\nFinished")
        data = pd.DataFrame(data).loc[:,["username", "product_id", "date"]].rename(columns={"username":"user_id", "product_id":"item_id"})
        ...
        print("Found preprocessed file: steam_reviews.tsv")

def main(path: str):
    review_tsv_file = os.path.join(path,"steam_reviews.tsv")
    ...
    processed.to_csv(
        f"{path}/steam_games_processed_view_full.tsv",
        sep="\t",
        index=False,
    )
    ...
    test.to_csv(
        f"{path}/steam_games_processed_view_test.tsv",
        sep="\t",
        index=False,
    )
    ...
    train.to_csv(
        f"{path}/steam_games_processed_view_train_full.tsv",
        sep="\t",
        index=False,
    )
    ...
    train2.to_csv(
        f"{path}/steam_games_processed_view_train_tr.tsv",
        sep="\t",
        index=False,
    )
    test2.to_csv(
        f"{path}/steam_games_processed_view_train_valid.tsv",
        sep="\t",
        index=False,
    )
    ...
    for curr_path in glob.glob(
        f"{path}/steam_games_processed_view*"
    ):
