...
def main(path: str):
    data = pd.read_csv(os.path.join(path,"All_Beauty_2014.csv"), sep=',', header=None, names=['user_id', 'item_id', 'rating', 'timestamp'], dtype={'user_id':str, 'item_id':str, 'rating':str, 'timestamp':int})
    ...
    processed.to_csv(
        f"{path}/amazon_beauty_processed_view_full.tsv",
        sep="\t",
        index=False,
    )
    ...
    test.to_csv(
        f"{path}/amazon_beauty_processed_view_test.tsv",
        sep="\t",
        index=False,
    )
    ...
    train.to_csv(
        f"{path}/amazon_beauty_processed_view_train_full.tsv",
        sep="\t",
        index=False,
    )
    ...
    train2.to_csv(
        f"{path}/amazon_beauty_processed_view_train_tr.tsv",
        sep="\t",
        index=False,
    )
    test2.to_csv(
        f"{path}/amazon_beauty_processed_view_train_valid.tsv",
        sep="\t",
        index=False,
    )
    ...
    for curr_path in glob.glob(
        f"{path}/amazon_beauty_processed_view*"
    ):
