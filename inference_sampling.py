...
def inference_with_sampling(args):
	...
	gru = gru4rec.GRU4Rec.loadmodel(args.model_path)
		...
		if not os.path.isfile(os.path.join('data', 'results', 'sampling_results', model_name + '_' + sname + '_rank.pickle')):
			...
			if sname == 'full':
				...
				if not os.path.isfile(os.path.join(test_path, negative_items_file + ".tsv")):
					...
					test_item_file_builder.create_test_items(gru, n=nsample, train_path=args.train_path, test_path=test_file_path, out_path_prefix=os.path.join(test_path, model_name))
